...
{
    Mat frame,frame_2,prvs[thread_num] ,cropped;

    pthread_t threads[thread_num];  dyanamic_params parts[thread_num];
    vector<double> vals;
    ...
    for(int i=0;i<thread_num;i++){       // first frame is projected and cropped and stored in prvs matrix
        prvs[i]=frame.clone()(roi_parts[i]);
    }
    ...
    while(true)
    {
       ...
       for(int i=0;i<thread_num;i++){
           ...
           parts[i]={cropped(roi_parts[i]),prvs[i]};
...
int main(int argc, char** argv)
{
    ...
    freopen("out_denseOpt_spatial_pthreads.txt","w",stdout); // file in which data will be written
