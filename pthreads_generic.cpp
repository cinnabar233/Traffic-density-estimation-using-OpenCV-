...
vector<double> run_queue_density(VideoCapture cap, Mat img,Mat h , int NUM_THREADS)
{
    Mat frame,frame_2,prvs,xframe,xbg; // left_frame, right_frame , left_bg , right_bg;
    
   //  Rect frames[NUM_THREADS];
   ...
   /* for(int i = 0 ; i < NUM_THREADS ; i++)
    {
       Rect frames[i](i*(544)/NUM_THREADS , 0 , 544/NUM_THREADS , 867);
    }*/
    //Rect   roi_left(0,0,272,867);
    //Rect roi_right(272,0,272,867);
    //left_bg = img(roi_left);
   // right_bg = img(roi_right);
    warpPerspective(frame,frame_2, h,Size(1920,1080));
    queue_params *x[NUM_THREADS] ;
    queue_params y ;
    frame = frame_2(roi);
    cvtColor(frame, prvs, COLOR_BGR2GRAY);     // first frame is projected and cropped and stored in prvs matrix
    int cnt = 0 ; // denotes the frame number
    ...
    while(true)
    {
        ...
        double area = 0 ;
        for(int i = 0 ; i < NUM_THREADS ; i++) area += x[i] -> area;
        /*left_frame = frame_2(roi_left);

        right_frame = frame_2(roi_right);

        queue_params *left_half , *right_half;
        queue_params r = {right_frame , right_bg} , l = {left_frame , left_bg};
        right_half = &r;
        left_half = &l;
        
        int l1 = pthread_create(&threads[0] , NULL , f , (void *)left_half);
        ...
        int r1 = pthread_create(&threads[1] , NULL , f , (void *)right_half);
     ...
     //   cout<<left_status<<endl;
       ...
       // pthread_join(threads[0] , NULL);
        ...
        //double left_status= left_half->area;
        //double right_status= right_half->area;
        
        //double area = left_status + right_status;
...
void generate(string vid, Mat img,Mat h , int NUM_THREADS)
{
    VideoCapture cap1(vid);
    vector<double> v1 = run_queue_density(cap1 , img , h , NUM_THREADS) ;
...
int main(int argc, char** argv)
{
    ...
    freopen("out_pthreads_generic_txt","w",stdout); // file in which data will be written
    cout<<"frame,queue density"<<"\n";

    generate(vid,bg,h,NUM_THREADS);  // function to generate data
    //out.close();
    // fclose(stdout);
